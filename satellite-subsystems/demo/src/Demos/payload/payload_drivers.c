...
SoreqResult payloadSendCommand(char opcode, int size, unsigned char *buffer, int delay) {
	int err = I2C_write(PAYLOAD_I2C_ADDRESS, (unsigned char *)&opcode, 1);
    if  ( err!= 0 )
    	return err;
    vTaskDelay(delay);
...
SoreqResult payloadTurnOff() {
    ...
    return isismepsv2_ivid7_piu__outputbuschanneloff(EPS_INDEX, isismepsv2_ivid7_piu__imeps_channel__channel_5v_sw3, &response) ? EPS_ERROR : PAYLOAD_SUCCESS;
}
