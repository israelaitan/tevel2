 ...
 *  	Author: OBAR
 *
 *  *Note* IVID7 demo only, for IVID7 only minor differences exist.
 * 			IVID7 interface should work with IVID7 hardware.
 *
...
static Boolean _reset( void )
{
	...
	int error = isismepsv2_ivid7_piu__reset(_index, &response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__reset(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _cancel( void )
{
	...
	int error = isismepsv2_ivid7_piu__cancel(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__cancel(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _watchdog( void )
{
	...
	int error = isismepsv2_ivid7_piu__resetwatchdog(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__watchdog(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _outputbuschannelon( void )
{
	...
	isismepsv2_ivid7_piu__imeps_channel_t obc_idx = INPUT_GetINT8("Single Output Bus Channel Index: ");

	int error = isismepsv2_ivid7_piu__outputbuschannelon(_index, obc_idx, &response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__outputbuschannelon(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _outputbuschanneloff( void )
{
	...
	int error = isismepsv2_ivid7_piu__outputbuschanneloff(_index, obc_idx, &response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__outputbuschanneloff(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _switchtonominal( void )
{
	...
	int error = isismepsv2_ivid7_piu__switchtonominal(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__switchtonominal(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _switchtosafety( void )
{
	...
	int error = isismepsv2_ivid7_piu__switchtosafety(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__switchtosafety(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _getsystemstatus( void )
{
	...
	int error = isismepsv2_ivid7_piu__getsystemstatus(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__getsystemstatus(...) return error (%d)!\n\r",error);
		return FALSE;
	}
	...
	if(response.fields.reset_cause > 7)
	{
		string_index = 7;
	}
...
{
	isismepsv2_ivid7_piu__gethousekeepingraw__from_t response;

	int error = isismepsv2_ivid7_piu__gethousekeepingraw(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__gethousekeepingraw(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
static Boolean _gethousekeepingeng( void )
{
	...
	int error = isismepsv2_ivid7_piu__gethousekeepingeng(_index,&response);
	if( error )
	{
		TRACE_ERROR("isismepsv2_ivid7_piu__gethousekeepingeng(...) return error (%d)!\n\r",error);
		return FALSE;
	}
...
void WDT_kick_task_piu7(void* arguments)
{
	isismepsv2_ivid7_piu__replyheader_t response;
	...
	while(1)
	{
		isismepsv2_ivid7_piu__resetwatchdog(_index, &response);
		vTaskDelay( WDT_KICK_PERIOD );
	}
}


static Boolean _start_WDT_kick_piu7(void)
{
	if(WDTTaskHandle == NULL)
	{
		xTaskCreate(WDT_kick_task_piu7, (const signed char*)"Task_WDT_piu", 1024, NULL, configMAX_PRIORITIES-2, &WDTTaskHandle);
	}
	else
	{
...
static Boolean selectAndExecuteIsis_EpsDemoTest(void)
{
	...
	printf("\t 7 - Output Bus Channel Off \n\r");
	...
	switch(selection)
	{
		...
		case 10:
			...
			break;
		case 11:
			offerMoreTests = _start_WDT_kick_piu7();
			break;
		case 12:
...
Boolean isismepsv2_ivid7_piu__demo__init(void)
{
    int retValInt = 0;
	...
	retValInt = ISISMEPSV2_IVID7_PIU_Init( subsystem, 1);
	if(!retValInt)
		printf("\nISIS_EPS subsystem have been initialized successfully.\n\r");
	else if(retValInt == driver_error_reinit)
		printf("\nISIS_EPS subsystem have already been initialized.\n\r");
...
void isismepsv2_ivid7_piu__demo__loop(void)
{
...
Boolean isismepsv2_ivid7_piu__demo__main(void)
{
	if(!isismepsv2_ivid7_piu__demo__init())
	{
		return FALSE;
	}

	isismepsv2_ivid7_piu__demo__loop();

	return TRUE;
}

Boolean isismepsv2_ivid7_piu__test(void)
{
	return isismepsv2_ivid7_piu__demo__main();
}
